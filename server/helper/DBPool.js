...
class DBPool {
    ...
    static connectionInfo = {
        ...
        user: process.env.DATABASE_USERNAME,    // mysql에 로그인 할 수 있는 계정
        ...
        connectTimeout: process.env.DATABASE_CONNECTION_TIMEOUT,    // 커넥션 타임아웃
    ...
    constructor() {
        ...
        this.pool.on('connection', (connection) => {
            logger.info(` >> DATABASE에 접속됨 [threaId = ${connection.threadId}]`);
        ...
        this.pool.on('acquire', (connection) => {
            logger.info(` >> DATABASE 임대됨 [threaId = ${connection.threadId}]`);
        });

        this.pool.on('release', (connection) => {
            logger.info(` >> DATABASE 반납됨 [threaId = ${connection.threadId}]`)
        });
    }

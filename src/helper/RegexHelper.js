...
class RegexHelper {
    ...
    /** 두 값이 동일한지 검사(값이 다르면 에러 - 비밀번호 변경에 사용) */
    compareTo2(origin, compare, msg) {
        this.value(origin, msg);
        this.value(compare, msg);
    ...
    /** 체크박스(1개)가 선택되었는지 검사한다. */
    agree(field, msg) {
        const checkedItem = field.checked;
     ...
     */
    engNum(field, msg) {
        return this.field(field, msg, /^.*(?=.*\d)(?=.*[a-zA-Z]).*$/);
    }

    /**
     * 영문과 숫자와 특수문자(special form)로 이루어 졌는지 검사하기 위해 filed()를 간접적으로 호출한다.
     * @param {HTMLElement} field   검사할 대상에 대한 <INPUT>요소의 DOM 객체
     * @param {string} msg          표시할 메세지
     */
